...
class Solver(BaseSolver):
    ...
    def exec(self):
        ...
        for s, ds in zip(['dev', 'test'], [self.dv_set, self.tt_set]):
            # for s, ds in zip(['test', 'dev'], [self.tt_set, self.dv_set]):
            # Setup output
            ...
            if self.greedy:
                ...
                self.verbose(
                    'Performing instance-wise CTC beam decoding on {} set, num of batch = {}.'.format(s, len(ds)))
                ...
                beam_decode_func = partial(beam_decode, model=copy.deepcopy(
                    self.decoder), device=self.device, job=s)
                # Parallel beam decode
...
def beam_decode(data, model, device, job):
    ...
    #     path = "/data01/AuFast/Pan_dataset/SE_asr/test2/se_fbank/gen_test_mat/"
